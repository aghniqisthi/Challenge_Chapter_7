package com.example.challengechapter7.marcelle.workers
    ...
    override fun doWork(): Result {
        ...
        return try {
            ...
            saveImageToMedia(applicationContext, inputData.getString(KEY_IMAGE_URI))?.let { imageUriStr: String ->
                ...
                // Log the error
                Log.e(ContentValues.TAG, "Error: Image could not be saved to MediaStore")
                // Return as failed
                Result.failure()
            }
        } catch (throwable: Throwable) {
            // Log the error
            Log.e(ContentValues.TAG, "Error occurred while saving image to MediaStore", throwable)
            // Return as failed
            Result.failure()
        }
    }
